...
class SupabaseClient:
    ...
    def verify_bucket(self) -> Dict[str, Any]:
        """
        Verify that the configured storage bucket exists
        Returns verification status and creates bucket if needed
        """
        try:
            logger.info(f"Verifying storage bucket: {settings.SUPABASE_STORAGE_BUCKET}")
            
            # Try to list the bucket to see if it exists
            buckets = self.client.storage.list_buckets()
            
            bucket_exists = any(bucket.name == settings.SUPABASE_STORAGE_BUCKET for bucket in buckets)
            
            if bucket_exists:
                logger.info(f"✅ Storage bucket '{settings.SUPABASE_STORAGE_BUCKET}' exists")
                return {
                    "success": True,
                ...
                logger.warning(f"⚠️ Storage bucket '{settings.SUPABASE_STORAGE_BUCKET}' does not exist")
                
                # Try to create the bucket
                try:
                    logger.info(f"Creating storage bucket: {settings.SUPABASE_STORAGE_BUCKET}")
                    create_result = self.client.storage.create_bucket(
                        settings.SUPABASE_STORAGE_BUCKET,
                        {"public": False}  # Private bucket for security
                    )
                    
                    logger.info(f"✅ Created storage bucket: {settings.SUPABASE_STORAGE_BUCKET}")
                    return {
                        "success": True,
                        "bucket_name": settings.SUPABASE_STORAGE_BUCKET,
                        "status": "created"
    ...
    async def upload_video_chunk(
        ...
        """
        Upload a video chunk to Supabase Storage with multi-tenant path structure
        
        Args:
            session_id: Recording session ID (UUID or string)
            chunk_index: Index of the chunk
            file_content: Binary content of the chunk
            content_type: MIME type of the content
            organization_id: Organization ID for multi-tenant isolation (optional for backward compatibility)
        ...
        try:
            ...
            bucket_status = self.verify_bucket()
            if not bucket_status["success"]:
                return {
                    "success": False,
                    "error": f"Storage bucket verification failed: {bucket_status.get('error', 'Unknown error')}",
                    "details": bucket_status
                }
            
            # Normalize types to strings for path construction
            session_id_str = str(session_id)
            
            # Construct multi-tenant file path
            if organization_id:
                org_id_str = str(organization_id)
                file_path = f"organizations/{org_id_str}/recordings/{session_id_str}/chunks/chunk_{chunk_index:04d}.webm"
            else:
                # Fallback for backward compatibility
                file_path = f"recordings/{session_id_str}/chunks/chunk_{chunk_index:04d}.webm"
            
            # Log upload attempt
            file_size_mb = len(file_content) / (1024 * 1024)
            logger.info(f"Uploading chunk {chunk_index} for session {session_id}: {file_size_mb:.2f} MB to {file_path}")
