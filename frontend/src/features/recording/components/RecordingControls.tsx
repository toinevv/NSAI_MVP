...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  handleRecordingCompleteRef.current = async (recordingId: string) => {
    ...
    // Always set isUploading to show processing state immediately
    setIsUploading(true)
      ...
      if (progress.totalChunks > 0 && progress.completedChunks < progress.totalChunks) {
        ...
        const handleQueueComplete = async (event: UploadEvent) => {
          if (event.type === 'queue_completed') {
            ...
            console.log('🎯 UPLOAD COMPLETE: Recording session completed, should trigger navigation')
          } else if (event.type === 'queue_failed') {
            console.error('Upload queue failed - completing recording with partial data')
            recordingAPI.offUploadEvent(handleQueueComplete)
            await completeRecordingSession(recordingId)
            console.log('🎯 UPLOAD FAILED: Recording session completed with errors, should trigger navigation')
          }
        }
  ...
  const completeRecordingSession = async (recordingId: string) => {
    ...
    try {
      ...
      // Call completion callback BEFORE resetting state to ensure sessionId is available
      console.log('🎯 Calling onRecordingComplete with sessionId:', sessionId)
      if (onRecordingComplete) {
        console.log('✅ onRecordingComplete callback exists, calling now')
        try {
          onRecordingComplete(sessionId)
          console.log('✅ onRecordingComplete callback called successfully')
        } catch (callbackError) {
          console.error('❌ Error in onRecordingComplete callback:', callbackError)
        }
      } else {
        console.warn('❌ onRecordingComplete callback is missing!')
      }
  ...
  const getStatusIcon = () => {
    switch (screenRecording.state.status) {
      ...
      case 'paused': return <div className="w-3 h-3 bg-yellow-500 rounded-full" />
      case 'stopping': return <Upload className="w-5 h-5 text-orange-500 animate-pulse" />
  ...
  return (
    <div className={`bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl shadow-xl border border-blue-200 p-8 max-w-4xl mx-auto ${className}`}>
      ...
      {(isActivelyRecording || isUploading || status === 'stopping') && (
        <div className="bg-gray-50 rounded-lg p-4 mb-6">
          ...
          {/* Status Message for Stopping */}
          {status === 'stopping' && !isUploading && (
            <div className="border-t pt-4">
              <div className="flex items-center justify-center space-x-2 text-orange-600">
                <Upload className="w-5 h-5 animate-pulse" />
                <span className="text-sm font-medium">
                  Recording stopped - preparing upload...
                </span>
              </div>
        ...
        {!isActivelyRecording ? (
          ...
          <div className="flex items-center space-x-4">
            {isRecording ? (
              ...
              <button
                onClick={screenRecording.resumeRecording}
                className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-teal-500 hover:from-blue-600 hover:to-teal-600 text-white rounded-full font-medium transition-all duration-300 hover:scale-105 shadow-lg"
              >
